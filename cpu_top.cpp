...
void copy_results_from_FPGA(network_t *net_CPU, data_t *results, int ch_out) {
  ...
  assert(net_CPU->layers[net_CPU->num_layers - 1].pool == POOL_GLOBAL);
...
void calculate_softmax(network_t *net_CPU, data_t *results,
  ...
  layer_t lastlayer = net_CPU->layers[net_CPU->num_layers - 1];
  data_t num_output_pixels = lastlayer.width * lastlayer.height;
  if (lastlayer.stride == 2) num_output_pixels = num_output_pixels / 4;
  int ch_out = lastlayer.channels_out;
